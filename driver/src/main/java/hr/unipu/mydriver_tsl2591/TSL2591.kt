...
class TSL2591(private var device: I2cDevice) : Closeable {
    ...
    private val observable: io.reactivex.subjects.Subject<Int> = PublishSubject.create<Int>()
    ...
    private fun startBackgroundThread(observable: io.reactivex.subjects.Subject<Int>) {
        if (handler == null) {
        ...
        if (runnable == null) {
            runnable = Runnable {
                if (isThreadRunning) {
                    ...
                    runnable?.let { handler!!.postDelayed(it, getTimeInMS(time)) }
                }
            }
        }
